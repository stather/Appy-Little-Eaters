...
- (void)updateBounds {
    ...
    topView.frame = CGRectMake(x, -1, width, y + 1);
    ...
    if (topLeftDot)
    {
        [topLeftDot removeFromSuperview];
        topLeftDot = nil;
    }
    
    topLeftDot = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"dragDot.png"]];
    [topLeftDot setFrame:CGRectMake(cropView.frame.origin.x-12.0f, cropView.frame.origin.y-12.0f, 25.0f, 25.0f)];
    [self.imageView addSubview:topLeftDot];
    
    if (topRightDot)
    {
        [topRightDot removeFromSuperview];
        topRightDot = nil;
    }
    
    topRightDot = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"dragDot.png"]];
    [topRightDot setFrame:CGRectMake(cropView.frame.origin.x+cropView.frame.size.width-12.0f, cropView.frame.origin.y-12.0f, 25.0f, 25.0f)];
    [self.imageView addSubview:topRightDot];

    if (bottomLeftDot)
    {
        [bottomLeftDot removeFromSuperview];
        bottomLeftDot = nil;
    }
    
    bottomLeftDot = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"dragDot.png"]];
    [bottomLeftDot setFrame:CGRectMake(cropView.frame.origin.x-12.0f, cropView.frame.origin.y+cropView.frame.size.height-12.0f, 25.0f, 25.0f)];
    [self.imageView addSubview:bottomLeftDot];
    
    if (bottomRightDot)
    {
        [bottomRightDot removeFromSuperview];
        bottomRightDot = nil;
    }
    
    bottomRightDot = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"dragDot.png"]];
    [bottomRightDot setFrame:CGRectMake(cropView.frame.origin.x+cropView.frame.size.width-12.0f, cropView.frame.origin.y+cropView.frame.size.height-12.0f, 25.0f, 25.0f)];
    [self.imageView addSubview:bottomRightDot];
...
//    
//    [topLeftView setBackgroundColor:[UIColor clearColor]];
//    [topRightView setBackgroundColor:[UIColor clearColor]];
...
//                }
//                else
                if (currentDragView == topLeftView) {
                    frame.size.width += CGOriginX(frame) - x;
                    frame.size.height += CGOriginY(frame) - y;
                    frame.origin = touch;
                }
