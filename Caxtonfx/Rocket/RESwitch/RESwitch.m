...
- (void)layoutSubviews
{
    ...
    knobFrame.origin.y = 2;//_knobOffset.height;
    _knobView.frame = knobFrame;
    
    
    CGRect frame1 = _backgroundView.frame;
    int x,y;
    if (_isOn) {
        NSLog(@"on");
        frame1.origin.x = 0;
    ...
    _backgroundView.frame = frame1;
    
    CGRect knobFrame1 = _knobView.frame;
    knobFrame1.origin.x = x;//frame.origin.x + self.frame.size.width - knobFrame.size.width + _knobOffset.width;
    knobFrame1.origin.y = y;
    _knobView.frame = knobFrame1;
...
- (void)setOn:(BOOL)on animated:(BOOL)animated
{
    ...
    if (!on) {
        ...
        knobFrame.origin.x = x;//frame.origin.x + self.frame.size.width - knobFrame.size.width + _knobOffset.width;
        knobFrame.origin.y = y;
        _knobView.frame = knobFrame;
        NSLog(@"_knobView.frame - >>>%@",NSStringFromCGRect(_knobView.frame));
...
- (void)panGestureRecognizerDidChange:(UIPanGestureRecognizer *)recognizer
{
    ...
    _backgroundView.frame = frame;
    NSLog(@"offPosition %f",self.offPosition);
    ...
    if (recognizer.state == UIGestureRecognizerStateEnded) {
        ...
        if (frame.origin.x > -self.frame.size.width / 2) {
            ...
            frame.origin.x = -65;//self.offPosition;
            knobX = -3;
            knobY =2;
        }
        ...
        [UIView animateWithDuration:0.1 animations:^{
            ...
            CGRect knobFrame = _knobView.frame;
            knobFrame.origin.x = knobX;//frame.origin.x + self.frame.size.width - knobFrame.size.width + _knobOffset.width;
            
             knobFrame.origin.y = knobY;
            _knobView.frame = knobFrame;
        }];
