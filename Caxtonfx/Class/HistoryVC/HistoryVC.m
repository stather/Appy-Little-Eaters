...
-(void)customizingNavigationBar
{
    ...
    UILabel *titleLbl = [[UILabel alloc] initWithFrame:CGRectMake(0.0f, 5.0f,320,30.0f)];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    ...
    else
    {
        ...
        else if((historyArray.count == 0) && (conversionArray.count != 0))
        {
            if (section == 0)
            {
                return ([conversionArray count]>3)?3:[conversionArray count];
...
-(UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    if (historyArray.count !=0 && conversionArray.count !=0)
    {
        if (section == 0) {
            ...
            sectionName = @"Latest card transactions";
    ...
    else
    {
        ...
        else if((historyArray.count == 0) && (conversionArray.count != 0))
        {
            if (indexPath.section == 0)
            {
                ...
                cell.globalRateLabel.text = @"Caxton FX Global Traveller Rate";
                cell.dateLabel.text= [dict objectForKey:@"date"];
                NSLog(@"[dict objectForKey:] %@",[dict objectForKey:@"conversionValue"]);
...
-(void)switchToTransactionVC
{
    if(historyArray.count>0)
    {
        ...
        tempController.historyArray = historyArray;
