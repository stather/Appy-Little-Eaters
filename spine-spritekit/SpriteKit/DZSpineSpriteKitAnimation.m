...
- (SKAction *) skActionForBone:(spBone *) bone timeline:(SpineTimeline *) timeline duration:(CGFloat) duration delay:(CGFloat) delay
{
	...
	SpineSequence* seq = [timeline sequencesForType:kSpineSequenceTypeBonesTranslate][0];
    if ( ![sequenceTypes containsObject:kSpineSequenceTypeBonesTranslate] || [seq time] != 0) {
        [actions addObject:[SKAction moveTo:geometry.origin duration:poseDelay]];
    }
	seq = [timeline sequencesForType:kSpineSequenceTypeBonesRotate][0];
    if ( ![sequenceTypes containsObject:kSpineSequenceTypeBonesRotate] || [seq time] != 0) {
        CGFloat radians = (CGFloat)(geometry.rotation * M_PI / 180);
        [actions addObject:[SKAction rotateToAngle:radians duration:poseDelay shortestUnitArc:YES]];
    }
	seq = [timeline sequencesForType:kSpineSequenceTypeBonesScale][0];
    if ( ![sequenceTypes containsObject:kSpineSequenceTypeBonesScale] || [seq time] != 0) {
